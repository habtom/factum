...
'''
	...
	def dispatch static generateFormula(CtaBinaryFormulas ctabf, Pair<String, String> pair){
		...
		if(ctabf.binaryOperator !== null)
		{
			...
			if(ctabf.binaryOperator.LAnd == '∧' && ctabf.right.ctaBaseTerms !== null && ctabf.right.ctaBaseTerms.ctaPredicateTerms !== null){
				equalityPair = generateFormula(ctabf.right.ctaBaseTerms.ctaPredicateTerms, pair) as Pair<String, String>;				
				result += '''«mapFormula(ctabf.left, equalityPair)»''';
			} else if(ctabf.binaryOperator.LAnd == '∧' && ctabf.right.ctaFormulaWithBracket !== null) {
				try{
					var ctaBinaryFormulas = ctabf.right.ctaFormulaWithBracket.ctaPrimaryFormula.ctaBinaryFormulas as CtaBinaryFormulas;
					if(ctaBinaryFormulas.ctaPrimary.ctaBaseTerms.ctaPredicateTerms !== null){
						equalityPair = generateFormula(ctaBinaryFormulas.ctaPrimary.ctaBaseTerms.ctaPredicateTerms, pair)  as Pair<String, String>;
						result += '''«mapFormula(ctabf.left, equalityPair)»''';
					} else{
	...
	def dispatch static generateFormula(CtaQuantifiedFormulas ctaq, Pair<String, String> pair){
		...
		if(ctaq.quantifierOperator !== null){
			if(ctaq.quantifierOperator.exists == '∃'){
				...
				throw new UnsupportedOperationException(ctaq.quantifierOperator + " quantified operator is not supported.");
			}
		}
	...
	def dispatch static Pair<String, String> generateFormula(CtaPredicateTerms ctat, Pair<String, String> pair){
		if(ctat.equalityOperator !== null){
			if(ctat.ctaPTerm1.cmpVariableRef !== null && ctat.ctaPTerm2.cmpVariableRef !== null){
				...
				if(ctat.ctaPTerm1.termOperatorFunction.trmOperator.name === ctat.ctaPTerm2.termOperatorFunction.trmOperator.name){
					try{
						var operatorName = ctat.ctaPTerm1.termOperatorFunction.trmOperator.name;
						var leftVar = ctat.ctaPTerm1.termOperatorFunction.trmOperands.get(0).dtTypeVars.name + "_" + operatorName;
						var rightVar = ctat.ctaPTerm2.termOperatorFunction.trmOperands.get(0).dtTypeVars.name + "_" + operatorName;
						return new Pair<String, String>(leftVar, rightVar);
					} catch(Exception e){
						return null;
					}					
		...
		if(ctapCact.CAct == 'cAct'){
			...
			if(!eventList.containsKey(eventName)){
				...
				var event = "event " + eventName + " before(" + eventParameters + ") : \n\t\t" 
							+ "execution(* " + objectType + "." + "new(..))" + targetObject + " && within(" + objectType + ")" + equalityOperator + " { }";
	...
	def dispatch static generateFormula(CtaPredicateConn ctaConn, Pair<String, String> pair) {
		...
		if(iPort.name.toLowerCase() != oPort.name.toLowerCase()){
			throw new IllegalArgumentException("Wrong usage of conn() operator. Input port name and Output port name must be same.");
		}
		...
		if(!eventList.containsKey(eventName)) {
			var event = "event " + eventName + " before(" + eventParameters + ") : \n\t\t" +
						"call(* "+ iPortObject.cmptypAssigned.name + "." + iPort.name + "(..))" + thisObject + 
						targetObject + " && within(" + oPortObjectType + ")" + equalityOperator + " { }";
	...
	def static String getParameters(Set<Entry<String, String>> entrySet, boolean showValue, boolean showKey, String namePrefix, String delimiter){
		var result = "";
		if(entrySet === null){
			return result;
		}
