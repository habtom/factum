...
//CTA Dispatches
	...
	def dispatch static generateFormula(CtaPredicateTerms ctat){
		...
		val ctpTerm2CmpTypPrt = ctat.ctaPTerm2.termOperatorFunction.trmOperands.map[cmpVariableRef.portRef.port.name].toString.replaceAll("[\\[\\],]","")
...
//AG dispatches
	...
	def dispatch static generateFormula(AgPredicateTerms agt){
		...
		val ctpTerm2CmpTypPrt = agt.agPTerm2.termOperatorFunction.trmOperands.map[cmpVariableRef.portRef.port.name].toString.replaceAll("[\\[\\],]","")
